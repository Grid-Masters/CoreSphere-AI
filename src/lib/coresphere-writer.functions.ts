...
const instructions: Record<WriterTool, string> = {
  ...
  "Banking language enhancement": "Improve clarity and elevate to precise banking-operations language. Do not introduce any procedure, fee, limit, SLA, authentication step or compliance rule that is not already in the supplied text.",
  "Memo generator": "Turn the input into a well-structured internal banking memo with subject, body and a clear call to action, using ONLY the facts supplied. Omit any section for which the supplied text provides no facts.",
  "Customer response generator": "Draft a courteous customer response using ONLY the facts supplied. Never state a timeline, fee, requirement, entitlement or next step that is not in the supplied text — omit it instead.",
  "SOP summarizer": "Summarize ONLY the supplied SOP content into key steps, owners and escalation points. If owners or escalation points are not stated in the supplied text, omit those items rather than inferring them.",
  "Policy summarizer": "Summarize ONLY the supplied policy text into plain-language key points and obligations. Do not add obligations, exceptions or regulatory references that are absent from the supplied text.",
